        ...
        def check_devices_available_actor():
            ...
            try:
                ...
                # Check if there are new port names
                    ...
                    self.port = port
                    publish_message(
                        f"New device found on port: {port}", "peripheral/info"
                    )
                ...
                self.port = None
                publish_message(
                    f"No device available for connection with exception {e}",
                    "peripheral/error",
                )
...
def disconnected_wrapper(f, *args, **kwargs):
    ...
    f(*args, **kwargs)
    publish_message("device disconnected", "peripheral/disconnected")
...
def serial_proxy_manager(port_name: Str, topic: Str, *args, **kwargs):
    ...
    try:
        ...
        proxy.monitor.disconnected_event.set = ft.partial(
            disconnected_wrapper, proxy.monitor.disconnected_event.set
        )
        time.sleep(2)
...
def main(args):
    ...
    message_router_actor.message_router_data.add_subscriber_to_topic(
        "peripheral/#", "print_message"
    )
    message_router_actor.message_router_data.add_subscriber_to_topic(
        "peripheral/port", "serial_proxy_manager"
    )
    message_router_actor.message_router_data.add_subscriber_to_topic(
        "peripheral/disconnected", "serial_proxy_manager"
    )
